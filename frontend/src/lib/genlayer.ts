...
export async function writeTx(functionName: string, args: string[], valueWei?: bigint) { if (!isConfigured) throw new Error('Contract not configured.'); await connectStudionet(); const client = getWriteClient(); try { return client.writeContract({ address: CONTRACT_ADDRESS as `0x${string}`, functionName, args, value: valueWei ?? 0n }) } catch (error) { const message = error instanceof Error ? error.message : 'Unknown wallet or RPC error.'; throw new Error(`Could not submit ${functionName}: ${message}`) } }
