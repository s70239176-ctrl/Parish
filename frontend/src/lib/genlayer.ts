...
export async function writeTx(functionName: string, args: string[], valueWei?: bigint) { if (!isConfigured) throw new Error('Contract not configured.'); await connectStudionet(); const client = getWriteClient(); const call = { address: CONTRACT_ADDRESS as `0x${string}`, functionName, args, ...(valueWei === undefined ? {} : { value: valueWei }) }; try { const quote = await client.estimateTransactionFeesForWrite(call); return client.writeContract({ ...call, fees: { distribution: quote.distribution, feeValue: quote.feeValue } }) } catch (error) { throw new Error(`Could not submit ${functionName}: ${errorMessage(error)}`) } }
export async function waitUntilFinal(hash: `0x${string}`) { const receipt = await getReadClient().waitForFinalization({ hash }); if (!isSuccessful(receipt)) throw new Error(`Transaction did not succeed: ${receipt.statusName}.`); return receipt }
